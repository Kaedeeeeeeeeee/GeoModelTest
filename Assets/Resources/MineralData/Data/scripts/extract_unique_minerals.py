...
def get_unique_minerals_from_csv(csv_file):
    """获取CSV中唯一的矿物种类，保持首次出现的顺序"""
...
def extract_and_map_unique_images(excel_file, csv_file, output_dir):
    """提取图片并按唯一矿物种类映射"""
    
    print("=" * 80)
    print("修正的Excel图片提取工具 - 唯一矿物映射")
    print("=" * 80)
    ...
    # 获取唯一矿物
    unique_minerals = get_unique_minerals_from_csv(csv_file)
    print(f"\\n发现唯一矿物种类: {len(unique_minerals)} 种")
    
    # 显示所有唯一矿物
    print("\\n唯一矿物列表:")
    for i, (mineral_name, info) in enumerate(unique_minerals.items(), 1):
        print(f"{i:2d}. {mineral_name} -> {info['mineral_id']} (首次出现: 行{info['first_row']})")
    ...
    if len(image_files) < len(unique_minerals):
        print(f"\\n⚠️  警告: Excel中图片数量({len(image_files)})少于唯一矿物数量({len(unique_minerals)})")
        print("将只处理前{}种矿物".format(len(image_files)))
    elif len(image_files) > len(unique_minerals):
        print(f"\\n⚠️  警告: Excel中图片数量({len(image_files)})多于唯一矿物数量({len(unique_minerals)})")
        print("将只使用前{}张图片".format(len(unique_minerals)))
    ...
    try:
        with zipfile.ZipFile(excel_file, 'r') as zip_file:
            for i in range(mapping_count):
                ...
                print(f"{i+1:2d}. {mineral_name:25} -> {new_filename}")
    
    except Exception as e:
        print(f"提取图片时出错: {e}")
        return
    
    print(f"\\n重命名完成: {mapping_count} 个图片")
...
def main():
    ...
    extract_and_map_unique_images(excel_path, csv_path, output_path)
