...
public class GeometricSampleReconstructor : MonoBehaviour
{
    ...
    private LayerSegment CreateLayerSegment(LayerIntersection intersection, int index, Transform parent, float segmentCenter, float thickness, float radius)
    {
        ...
        // 直接使用地层的原始材质
        Material segmentMaterial = GetOriginalLayerMaterial(intersection.layer);
    ...
    /// 直接获取地层的原始材质
    /// </summary>
    private Material GetOriginalLayerMaterial(GeologyLayer layer)
    {
        // 直接使用地层的sharedMaterial，就像钻塔系统一样
        MeshRenderer meshRenderer = layer.GetComponent<MeshRenderer>();
        if (meshRenderer != null && meshRenderer.sharedMaterial != null)
        {
            return meshRenderer.sharedMaterial; // 直接返回，不创建新材质
        }
        
        // 如果没有MeshRenderer，创建一个简单的材质
        Material fallbackMaterial = new Material(Shader.Find("Standard"));
        fallbackMaterial.color = layer.layerColor;
        fallbackMaterial.name = $"{layer.layerName}_GeneratedMaterial";
    ...
    /// 创建地层材质 - 保留旧方法作为备用
    /// </summary>
    [System.Obsolete("请使用GetOriginalLayerMaterial方法")]
    private Material CreateLayerMaterial(GeologyLayer layer, int segmentIndex)
    {
        return GetOriginalLayerMaterial(layer);
    }
