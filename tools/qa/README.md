...
- `loading-server.py --preview`: render the current WebGL template against existing build files at `http://127.0.0.1:55888/normal/`. Omit `--preview` to test the finished release. Other URL prefixes simulate unavailable scripts, HTTP errors, malformed data, interrupted streams, a temporary pause, and a 90-second timeout. This is a localhost-only fault server.
- `loading-browser.py --long`: run the actual Unity loader through those failures and recovery, including mobile touch retry, cached reload, repeated failure, and preservation of synthetic localStorage/IndexedDB data. The long test waits for the real production inactivity threshold. Browser state and fault screenshots remain in ignored `Logs/loading-improvements/browser`.
- `WebGLAssetReport.Export` (Unity Editor): export the previous build's per-asset packed sizes to `Logs/webgl/asset-sizes.json`; `WebGLBuildSetup.BuildWebGL` also exports this after every build. Packed sizes are not the same as transferred gzip bytes; compare the final build files to measure download savings.
